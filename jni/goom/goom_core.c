...
/*
extern "C" {
    JNIEXPORT void JNICALL Java_br_com_belocodigo_goom2k4_Goom2k4_init(JNIEnv * env, jobject obj,  jint resx, jint resy);
    JNIEXPORT void JNICALL Java_br_com_belocodigo_goom2k4_Goom2k4_update(JNIEnv * env, jobject obj);
    JNIEXPORT void JNICALL Java_br_com_belocodigo_goom2k4_Goom2k4_close(JNIEnv * env, jobject obj);
};
*/

JNIEXPORT void JNICALL Java_br_com_belocodigo_goomandroid_Goom2k4_init(JNIEnv * env, jobject obj,  jint resx, jint resy)
{
	size = resx * resy;
	jApixels = (*env)->NewIntArray(env, size);
	populateData();
	goominfo = goom_init (resx, resy);
}

JNIEXPORT jintArray JNICALL Java_br_com_belocodigo_goomandroid_Goom2k4_update(JNIEnv * env, jobject obj)
{
	//goom_set_screenbuffer(goominfo, pixels);
	if (jApixels == NULL) {
	...
	goom_update (goominfo, data, 0, -1, t, t);

	for (i = 0; i < size; i++) {
		pixels[i] = goominfo->outputBuf[i].val;
	}

	(*env)->SetIntArrayRegion(env, jApixels, 0, size, pixels);
	return jApixels;
}

JNIEXPORT void JNICALL Java_br_com_belocodigo_goomandroid_Goom2k4_close(JNIEnv * env, jobject obj)
{
	goom_close(goominfo);
}
